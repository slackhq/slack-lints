...
class MoshiUsageDetector : Detector(), SourceCodeScanner {
  ...
  override fun createUastHandler(context: JavaContext): UElementHandler {
    return object : UElementHandler() {
      override fun visitClass(node: UClass) {
        ...
        // This can sometimes come back as the default empty String and sometimes as null if not defined
        ...
        val generateAdapter = jsonClassAnnotation.findAttributeValue(
          "generateAdapter"
        )
        ...
        val isUnsupportedType = node.isKotlinObject() ||
        ...
        val primaryConstructor = node.constructors
          .asSequence()
        ...
        for (parameter in primaryConstructor.uastParameters) {
          ...
          val defaultValueExpression = if (sourcePsi is KtParameter) {
            sourcePsi.defaultValue
          } else {
            null
          }
          val hasDefaultValue = defaultValueExpression != null
          if (parameter.uAnnotations.any { it.qualifiedName == "kotlin.jvm.Transient" } && !hasDefaultValue) {
          ...
          if (sourcePsi is KtParameter && sourcePsi.isPropertyParameter()) {
            ...
            val shouldCheckPropertyType = propertyAdaptedByAnnotation == null &&
              // If any JsonQualifier annotations are present, defer to whatever adapter looks at them
            ...
            val jsonAnnotations = (parameter.sourcePsi as KtParameter).annotationEntries
              .mapNotNull { annotationEntry ->
                (annotationEntry.toUElement() as KotlinUAnnotation)
                  .takeIf { it.qualifiedName == FQCN_JSON }
              }
            ...
            if (jsonAnnotations.isNotEmpty()) {
              if (jsonAnnotations.size != 1) {
                ...
                val annotationToKeep = jsonAnnotations.find {
                  it.sourcePsi.useSiteTarget == null
                } ?: jsonAnnotations[0]
                for (annotation in jsonAnnotations) {
                  ...
                  context.report(
                    ...
                    quickfixData = fix()
                      .removeNode(context, annotation.sourcePsi)
                  )
                }
              } else {
  ...
  private fun validateAdaptedByAnnotation(context: JavaContext, adaptedByAnnotation: UAnnotation) {
    ...
    val adapterAttribute = (
      adaptedByAnnotation.findAttributeValue(
        "adapter"
    ...
    val implementsAdapter = isInheritor(targetClass, true, FQCN_JSON_ADAPTER) ||
  ...
  private fun checkMoshiType(
    ...
    if (psiType is PsiArrayType) {
      ...
      val componentTypeName = if (componentType is PsiPrimitiveType) {
        componentType.boxedTypeName!!.let {
          if (it == "java.lang.Integer") "Int" else it.removePrefix("java.lang.")
        }
      } else {
        typeNode.sourcePsi!!.text
          .trim()
          .removePrefix("Array<").removeSuffix(">")
      }
    ...
    val psiClass = context.evaluator.getTypeClass(psiType)
      ?: error("Could not load class for ${psiType.className} on ${parameter.getUastParentOfType<UClass>()!!.name}.${parameter.name}")
    ...
    if (psiClass.isEnum) {
      ...
      } else if (defaultValueExpression is KtReferenceExpression ||
    ...
    // Check collections
    val isJsonCollection = psiClass.qualifiedName == "java.util.List" ||
    ...
    if (isJsonCollection) {
      ...
      val correctedImmutableType = when {
    ...
    when {
      ...
      isInheritor(psiClass, "java.util.Map") -> {
        ...
        context.report(
          ...
          context.getLocation(typeNode),
          ISSUE_NON_MOSHI_CLASS_MAP.getBriefDescription(TextFormat.TEXT)
            .withHint(psiClass.name),
          quickfixData = fix()
            .replace()
            .name("Change to Map")
  ...
  private fun checkJsonQualifierAnnotation(context: JavaContext, node: UClass) {
    ...
    node.findAnnotation(retentionAnnotationPair.first)?.let { retentionAnnotation ->
      val retentionValue = retentionAnnotation.findAttributeValue(retentionAnnotationPair.second)
    ...
    node.findAnnotation(targetAnnotationPair.first)?.let { targetAnnotation ->
      val targetValues = when (
        val targetsAttr =
          targetAnnotation.findAttributeValue(targetAnnotationPair.second)!!
      ) {
        is UCallExpression -> {
   ...
   *
   * This lint will attempt to detect if an enum is used with Moshi and check these
   * requirements. They should not generate adapters (i.e. `JsonClass.generateAdapter`
   * should be `false`) but are exempt from that requirement and the requirements
   * of this lint in general if they define a custom `JsonClass.generator` value.
   */
  private fun checkEnum(context: JavaContext, node: UClass) {
    ...
    if (jsonClassAnnotation != null) {
      ...
      // and we choose to ignore it in that case.
      // This can sometimes come back as the default empty String and sometimes as null if not defined
      val generator = jsonClassAnnotation.findAttributeValue("generator")
        ?.evaluate() as? String
  ...
  companion object {
    ...
    private val REQUIRED_VISIBILITIES = setOf(
    ...
    private val ISSUE_MISSING_TYPE_LABEL = createIssue(
      "MissingTypeLabel",
    ...
    private val ISSUE_BLANK_GENERATOR = createIssue(
      "BlankGenerator",
    ...
    private val ISSUE_PRIVATE_PARAMETER = createIssue(
    ...
    private val ISSUE_TRANSIENT_NEEDS_INIT = createIssue(
      "TransientNeedsInit",
    ...
    private val ISSUE_INAPPROPRIATE_TYPE_LABEL = createIssue(
    ...
    private val ISSUE_DOUBLE_TYPE_LABEL = createIssue(
      "DoubleTypeLabel",
    ...
    private val ISSUE_GENERIC_SEALED_SUBTYPE = createIssue(
    ...
    private val ISSUE_DOUBLE_CLASS_ANNOTATION = createIssue(
    ...
    private val ISSUE_VISIBILITY = createIssue(
      "ClassVisibility",
    ...
    private val ISSUE_PARAM_NEEDS_INIT = createIssue(
      "ParamNeedsInit",
    ...
    private val ISSUE_BLANK_JSON_NAME = createIssue(
      "BlankJsonName",
    ...
    private val ISSUE_REDUNDANT_JSON_NAME = createIssue(
      "RedundantJsonName",
    ...
    private val ISSUE_SERIALIZED_NAME = createIssue(
      "SerializedName",
    ...
    private val ISSUE_QUALIFIER_RETENTION = createIssue(
      "QualifierRetention",
    ...
    private val ISSUE_QUALIFIER_TARGET = createIssue(
      "QualifierTarget",
    ...
    private val ISSUE_JSON_SITE_TARGET = createIssue(
      "RedundantSiteTarget",
    ...
    private val ISSUE_OBJECT = createIssue(
      "Object",
    ...
    private val ISSUE_USE_DATA = createIssue(
      "UseData",
    ...
    private val ISSUE_UNSUPPORTED_TYPE = createIssue(
      "UnsupportedType",
    ...
    private val ISSUE_ADAPTED_BY_REQUIRES_ADAPTER = createIssue(
    ...
    private val ISSUE_ADAPTED_BY_REQUIRES_KEEP = createIssue(
    ...
    private val ISSUE_MISSING_PRIMARY = createIssue(
      "MissingPrimary",
    ...
    private val ISSUE_ENUM_PROPERTY_COULD_BE_MOSHI = createIssue(
    ...
    private val ISSUE_ENUM_PROPERTY_DEFAULT_UNKNOWN = createIssue(
    ...
    private val ISSUE_VAR_PROPERTY = createIssue(
      "VarProperty",
    ...
    private val ISSUE_SNAKE_CASE = createIssue(
      "SnakeCase",
    ...
    private val ISSUE_DUPLICATE_JSON_NAME = createIssue(
      "DuplicateJsonName",
    ...
    private val ISSUE_NON_MOSHI_CLASS_PLATFORM = createIssue(
    ...
    private val ISSUE_ARRAY = createIssue(
      "Array",
    ...
    private val ISSUE_MUTABLE_COLLECTIONS = createIssue(
      "MutableCollections",
    ...
    private val ISSUE_NON_MOSHI_CLASS_COLLECTION = createIssue(
    ...
    private val ISSUE_NON_MOSHI_CLASS_MAP = createIssue(
      "NonMoshiClassMap",
    ...
    private val ISSUE_NON_MOSHI_CLASS_INTERNAL = createIssue(
    ...
    private val ISSUE_NON_MOSHI_CLASS_EXTERNAL = createIssue(
    ...
    val ISSUE_ENUM_JSON_CLASS_GENERATED = createIssue(
    ...
    val ISSUE_ENUM_ANNOTATED_UNKNOWN = createIssue(
    ...
    val ISSUE_ENUM_UNKNOWN = createIssue(
      "EnumMissingUnknown",
    ...
    val ISSUE_ENUM_JSON_CLASS_MISSING = createIssue(
    ...
    val ISSUE_ENUM_CASING = createIssue(
      "EnumCasing",
    ...
    fun issues(): Array<Issue> = arrayOf(
      ISSUE_ADAPTED_BY_REQUIRES_ADAPTER,
      ISSUE_ADAPTED_BY_REQUIRES_KEEP,
      ISSUE_ARRAY,
      ISSUE_BLANK_GENERATOR,
